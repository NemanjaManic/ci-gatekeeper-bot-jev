...
- [ ] T002 Initialize the npm package: `package.json` (name
...
- [ ] T003 [P] Add npm scripts to `package.json`: `build` (`ncc build
      ...
      `quickstart.md`.
- [ ] T004 [P] Create `action.yml` at the repository root with the inputs,
...
- [ ] T005 [P] Define shared TypeScript types in `src/types.ts` for
...
- [ ] T006 [P] Implement `src/github.ts` read functions: fetch the PR diff,
...
- [ ] T007 [P] Implement `src/github.ts` write functions: `postOrUpdateComment(body: string)`
...
- [ ] T008 [P] Implement `src/metrics.ts`: a `recordDecisionLogEntry` function
...
- [ ] T009 Implement `src/config.ts`: load `RiskConfiguration` with precedence
      ...
      caching across runs."
- [ ] T010 Implement `src/jev.ts` base call: invoke Jev via
...
- [ ] T011 Implement `src/jev.ts` failure fallback: on call failure/timeout,
...
- [ ] T012 Implement `src/index.ts` orchestration skeleton: read the event via
...
- [ ] T013 [P] [US1] Create fixture `tests/fixtures/trivial-docs.diff` (a
...
- [ ] T014 [P] [US1] Write the fixture pipeline test in
...
- [ ] T015 [US1] In `src/index.ts`, confirm/complete the `route ==
...
- [ ] T016 [US1] Run T014 and confirm it passes; fix any gap between T012's
...
- [ ] T017 [P] [US2] Create fixture `tests/fixtures/auth-change.diff` (modifies
      an authentication module) with a mocked `JevTriageResponse` of
      `risk: "blocking"`.
- [ ] T018 [P] [US2] Create fixture `tests/fixtures/ci-config-change.diff`
...
- [ ] T019 [P] [US2] Create fixture `tests/fixtures/mixed-trivial-and-ci.diff`
      combining a doc change and a CI-config change in one diff.
- [ ] T020 [US2] Write fixture pipeline tests in
...
- [ ] T021 [US2] Implement risk-to-route escalation in `src/jev.ts`: combine
...
- [ ] T022 [US2] Implement "maximum risk across changed files" resolution: if
...
- [ ] T023 [US2] Wire `src/index.ts` to use the escalated route (T021) instead
...
- [ ] T024 [P] [US3] Create fixture `tests/fixtures/secret-looking-diff.diff`
      with a mocked `JevTriageResponse` of `touches_secrets: true`.
- [ ] T025 [US3] Write fixture pipeline tests in
...
- [ ] T026 [US3] Implement the comment body builder in `src/github.ts` (or a
...
- [ ] T027 [US3] Wire `src/index.ts` to call the T026 builder with the full
...
- [ ] T028 [P] [US4] Extend the T017 (auth-change) fixture scenario with a
      ...
      posted comment.
- [ ] T029 [P] [US4] Add a fixture asserting that the T013 (trivial,
...
- [ ] T030 [P] [US4] Add a fixture with a mocked Gemini call failure for a
...
- [ ] T031 [US4] Implement `src/fallback-review.ts`: call a Gemini model via
...
- [ ] T032 [US4] Wire `src/index.ts` to call `src/fallback-review.ts` (T031)
...
- [ ] T033 [US4] Extend the T026 comment builder call site (T027) to include
...
- [ ] T034 [P] [US5] Write a unit test in `tests/unit/config.test.ts`
...
- [ ] T035 [US5] Write a fixture pipeline test in
...
- [ ] T036 [US5] Verify/complete `src/config.ts` (T009) reads
...
- [ ] T037 [P] Write unit tests in `tests/unit/jev-mapping.test.ts` for the
      ...
      (T011).
- [ ] T038 [P] Write a unit test in `tests/unit/secret-redaction.test.ts`
...
- [ ] T039 Add a top-level `README.md` documenting the action's inputs/outputs
